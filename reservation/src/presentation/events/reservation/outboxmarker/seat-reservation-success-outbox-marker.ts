...
@Injectable()
export class SeatReservationSuccessEventOutboxMarker extends KafkaBaseConsumer {
  constructor(
    @Inject('IReservationOutboxWriter')
    private readonly reservationOutboxWriter: IReservationOutboxWriter,
  ) {
    super(
      OutboxMarkerGroup.SEAT_RESERVATION_SUCCESS_OUTBOX_MARKER_GROUP,
  ...
  async handleMessage(message: any): Promise<void> {
    await this.reservationOutboxWriter.updateSuccess({
      ...
      eventType: ConsumerType.SEAT_RESERVATION_SUCCESS,
    });
  }
}
